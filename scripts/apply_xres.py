...
def replace_in_file(fname, replacement,
                    begin='# --- APPLY XRES BEGIN',
                    end='# --- APPLY XRES END'):
    f = open(fname, 'r')
    lines = f.readlines()
    idx = sidx = eidx = 0
    ...
    if not begin:
        print('Error: %s: could not find beginning line' % fname)
    elif not end:
        print('Error: %s: could not find end line' % fname)
    else:
        while eidx >= sidx:
            lines.pop(eidx)
            eidx -= 1
...
for k in colors:
    repl.append('set $%s #%s' % (k, colors[k]))
...
open_tag = '\t<entry name="%s" mtime="%d" type="string">\n'
str_val = '\t\t<stringvalue>%s</stringvalue>\n'
close_tag = '\t</entry>\n'

repl += open_tag % ('foreground_color', now)
repl += str_val % double_color(colors['c_foreground'])
repl += close_tag

repl += open_tag % ('background_color', now)
repl += str_val % double_color(colors['c_background'])
repl += close_tag

repl += open_tag % ('palette', now)
repl += str_val % ':'.join(arr)
repl += close_tag

replace_in_file(
    home + '/.gconf/apps/gnome-terminal/profiles/Default/%gconf.xml', repl
)
...
for k in colors:
    arr.append('%s="%s"' % (k, colors[k]))

replace_in_file(home + '/scripts/rofi.sh', '\n'.join(arr))
